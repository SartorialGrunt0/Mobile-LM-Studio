...
api.MapPost("/settings", (SettingsUpdateRequest request, LmStudioConnectionService connectionService, PinSecurityService pinSecurity, LmStudioSettingsStore settingsStore, ILogger<Program> logger) =>
{
    ...
    try
    {
        security = pinSecurity.BuildUpdatedSecurity(request.RequireLogin, request.Pin);
    }
    catch (ArgumentException argEx)
    {
        return Results.ValidationProblem(new Dictionary<string, string[]> { ["pin"] = [argEx.Message] });
    }

    try
    {
        settingsStore.Save(normalizedSettings, security);
        connectionService.UpdateSettings(normalizedSettings.BaseUrl, normalizedSettings.ApiToken, normalizedSettings.McpConfigPath);
        pinSecurity.UpdateSecurity(security);
...
{
    var deleted = await repository.DeleteChatAsync(chatId, cancellationToken);
    return deleted ? Results.NoContent() : Results.NotFound();
});

api.MapPost("/chats/{chatId}/auto-title", async Task<IResult> (string chatId, AutoTitleRequest request, ChatRepository repository, LmStudioClient client, ILogger<Program> logger, CancellationToken cancellationToken) =>
{
    try
    {
        var chat = await repository.GetChatRecordAsync(chatId, cancellationToken);
        if (chat is null)
        {
            return Results.NotFound();
        }

        var title = await client.GenerateTitleAsync(chat.ModelKey, request.Input, cancellationToken);
        if (!string.IsNullOrWhiteSpace(title))
        {
            await repository.UpdateTitleAsync(chatId, title, cancellationToken);
        }
        return Results.Ok(new { title });
    }
    catch (Exception exception)
    {
        logger.LogWarning(exception, "Unable to auto-title chat {ChatId}.", chatId);
        return Results.Ok(new { title = (string?)null });
    }
});

api.MapPost("/chats/{chatId}/retry/stream", async Task (string chatId, HttpContext context, ChatRepository repository, McpCatalogService catalog, LmStudioClient client, CancellationToken cancellationToken) =>
{
    var retryContext = await repository.GetRetryContextAsync(chatId, cancellationToken);
    ...
    var assistant = ChatStreamHelpers.BuildAssistantPersistence(finalResponse, retryContext.Model, retryContext.ContextLength);
    await repository.SaveAssistantMessageAsync(chatId, retryRequest, assistant, cancellationToken);
});
...
api.MapPost("/chats/stream", async Task (HttpContext context, ChatStreamRequest request, ChatRepository repository, McpCatalogService catalog, LmStudioClient client, CancellationToken cancellationToken) =>
{
    ...
    var assistant = ChatStreamHelpers.BuildAssistantPersistence(finalResponse, request.Model, request.ContextLength);
...
internal static class ChatStreamHelpers
{
    ...
    internal static AssistantPersistenceResult BuildAssistantPersistence(LmStudioChatResponse response, string modelKey, int? contextLength)
    {
